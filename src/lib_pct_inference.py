...
PCTi.BeliefUpdating.argtypes =  [c_void_p, c_int, POINTER(c_char_p), POINTER(c_char_p), POINTER(c_char_p)]
...
def belief_updating(network, target_nodes, evidence_nodes, evidences, algorithm="Lauritzen"):
	...
	c_probs = PCTi.BeliefUpdating(network.network, algorithm, c_target_nodes, c_evidence_nodes, c_evidences)
	probs = []
	innerprobs = []
	for prob in c_probs:
		if prob == -3:
...
def P1(network, target_node, evidence_nodes):
	...
	outcome_combinations = itertools.product(*outcome_lists)
